...
function QuestionCard({
  ...
  async function handleSubmitAnswer() {
    ...
    try {
      if (question.answer) {
        await editCountryAnswer(question.answer.id, trimmed);
      } else {
        await submitCountryAnswer(question.id, authorProfileId!, trimmed);
      }
      setEditing(false);
      onChanged();
    } catch {
  ...
  async function handleHide() {
    ...
    setError(null);
    try {
      await hideCountryQuestion(question.id);
      onChanged();
    } catch {
...
export default function QuestionsSection({
  ...
  countryId: string;
  initialQuestions: CountryQuestion[];
}) {
  ...
  async function refresh() {
    try {
      const data = await getCountryQuestions(countryId);
      setQuestions(data);
    } catch (err) {
  ...
  async function handleAsk(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await askCountryQuestion(countryId, user.id, trimmed);
      setNewQuestion("");
      await refresh();
    } catch {
      ...
      {!authLoading && user ? (
        ...
        >
          ...
          <textarea
            ...
            placeholder="Pergunte algo sobre este país..."
        ...
        !authLoading && (
          <div className="rounded-xl bg-branco p-4 text-sm text-oliva">
            ...
            para fazer uma pergunta sobre este país.
          </div>
        )
      )}
